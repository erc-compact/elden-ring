...
# Set up logging for Condor diagnostics
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s', filename='rfi_mitigation.log')
logger = logging.getLogger(__name__)

# Wrapper function to track memory and execution stage in key areas
def log_stage(stage_name):
    import psutil
    memory = psutil.Process().memory_info().rss / (1024 * 1024)  # in MB
    logger.debug(f"Stage: {stage_name}, Memory Usage: {memory:.2f} MB")

# Usage Example: log_stage("Data Load Start") in main data loading section
