...
def get_file_header(file_path):
    ...
    if file_path.endswith(".fil"):
        ...
        try:
            ra = (
                fil.header.ra_deg
                if hasattr(fil.header, "ra_deg")
                else fil.header.src_ra
            )
            dec = (
                fil.header.dec_deg
                if hasattr(fil.header, "dec_deg")
                else fil.header.src_dec
            )
        except AttributeError:
            try:
                ra = fil.header.tstart_RA if hasattr(fil.header, "tstart_RA") else 0.0
                dec = (
                    fil.header.tstart_DEC if hasattr(fil.header, "tstart_DEC") else 0.0
                )
            except AttributeError:
                ra = 0.0
                dec = 0.0
...
def read_filterbank_file(file_path, start_time, end_time):
    ...
    if n_samples <= 0:
        raise ValueError(
            "Invalid time range: Start time must be less than end time within the file's duration."
        )
...
def calculate_statistics(dynamic_spectrum, time_bins_per_block):
    ...
    reshaped_spectrum = dynamic_spectrum[: n_blocks * time_bins_per_block].reshape(
        (n_blocks, time_bins_per_block, -1)
    )
    ...
    print(time_bins)
    sk_variance = (24 * time_bins * (time_bins - 1) ** 2) / (
        (time_bins - 3) * (time_bins - 2) * (time_bins + 3) * (time_bins + 5)
    )
    sk_variance = np.full_like(sk, sk_variance)

    skewness = (
        np.mean((reshaped_spectrum - mean[:, np.newaxis, :]) ** 3, axis=1) / std_dev**3
    )
...
def identify_frequent_outliers(
    outliers_sk, frequencies, threshold=0.7, neighbor_threshold=0.5
):
    occurrence_count = np.sum(outliers_sk, axis=0)
    frequent_outliers = np.where(occurrence_count > (threshold * outliers_sk.shape[0]))[
        0
    ]
    ...
    left_neighbors = left_neighbors[
        (occurrence_count[left_neighbors] > (neighbor_threshold * outliers_sk.shape[0]))
    ]
    ...
    right_neighbors = right_neighbors[
        (
            occurrence_count[right_neighbors]
            > (neighbor_threshold * outliers_sk.shape[0])
        )
    ]

    expanded_frequent_outliers = np.unique(
        np.concatenate((expanded_frequent_outliers, left_neighbors, right_neighbors))
    )
    ...
    frequency_ranges_in_mhz = [
        (
            frequencies[expanded_frequent_outliers[start]],
            frequencies[expanded_frequent_outliers[end - 1]],
        )
...
def plot_dynamic_spectrum_block(
    ...
    im1 = axs[0].imshow(
        dynamic_spectrum_block[block_idx].T,
        aspect="auto",
        cmap="viridis",
        origin="lower",
        extent=extent,
    ...
    axs[1].set_title(
        f"Dynamic Spectrum - Block {block_idx} (Frequent Outliers + Neighbors)"
    )
...
def process_interval(
    ...
    print(
        f"Running analysis for interval {interval_idx + 1}/{total_intervals}: Start time = {start_time}, End time = {end_time}"
    )
    ...
    (
        ...
        max_freq_mhz,
        sample_time_us,
    ) = read_data_file(file_path, start_time, end_time, file_type)
    ...
    reshaped_spectrum = dynamic_spectrum[: n_blocks * bins_per_target, :].reshape(
        (n_blocks, bins_per_target, n_channels)
    )
    ...
    reshaped_spectrum, mean, std_dev, sk, sk_variance, skewness, q1, q2, q3, q4 = (
        calculate_statistics(dynamic_spectrum, time_bins_per_block)
    )
    ...
    frequent_outliers_sk, expanded_frequent_outliers_indices = (
        identify_frequent_outliers(outliers_sk, frequencies)
    )
    ...
    FREQUENT_OUTLIERS_FILE = os.path.join(
        interval_output_folder, "frequent_outliers.txt"
    )
    ...
    for block_idx in range(reshaped_blocks.shape[0]):
        plot_dynamic_spectrum_block(
            ...
            block_idx,
            outliers_sk,
            interval_output_folder,
            target_resolution_ms * 1e3,
            min_freq_mhz,
            max_freq_mhz,
            n_channels,
            sk,
...
def plot_combined_sk_heatmap_and_histogram(
    ...
    axs[1].bar(
        frequencies_list, counts_list, color="black", alpha=0.7, label="Outliers"
    )
    ...
    for start, end in frequent_outliers_sk:
        ...
        frequent_mask |= mask
    axs[1].bar(
        frequencies_list[frequent_mask],
        counts_list[frequent_mask],
        color="orange",
        alpha=0.7,
        label="Outliers with 70% occurrence + their neighbors",
    )
    ...
    cax = axs[2].imshow(
        ...
        extent=extent,
        interpolation="none",
        vmin=np.nanquantile(sk, 0.02),
        vmax=np.nanquantile(sk, 0.98),
    )
    ...
    block_data = [
        (block, percent_outliers_per_block[block])
        for block in blocks_with_high_outliers
    ]
    ...
    for block, percent in block_data:
        ...
        axs[2].plot(
            [frequencies[0] - 5, frequencies[0]], [block, block], color=color, lw=2
        )  # Left segment
        axs[2].plot(
            [frequencies[-1], frequencies[-1] + 5], [block, block], color=color, lw=2
        )  # Right segment
...
def plot_combined_sk_heatmap_and_histogram_interval(
    ...
    output_file = os.path.join(
        output_folder, "combined_sk_heatmap_and_histogram_interval.png"
    )
    plt.tight_layout()
    plt.savefig(output_file, bbox_inches="tight")
    plt.close()
...
def run_analysis_for_intervals(
    file_path,
    output_folder,
    ...
    observation_end_time = observation_start_time + TimeDelta(
        total_duration, format="sec"
    )
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for future in futures:
            try:
                sk, sk_variance, outliers_sk, frequent_outliers_sk, frequencies = (
                    future.result()
                )
    ...
    combined_outliers_file = os.path.join(
        output_folder, "combined_frequent_outliers.txt"
    )
    save_outliers_to_file(
        merged_frequent_outliers, combined_outliers_file, is_freq=True
    )

    plot_combined_sk_heatmap_and_histogram(
        concatenated_sk,
        concatenated_sk_variance,
        frequencies,
        concatenated_outliers_sk,
        merged_frequent_outliers,
        output_folder,
        azimuth_start,
        elevation_start,
        azimuth_end,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process FITS or filterbank file and analyze RFI."
    )
    parser.add_argument(
        "input_file", type=str, help="Path to the FITS or filterbank file."
    )
    parser.add_argument(
        "output_folder", type=str, help="Output folder for plots and results."
    )
    parser.add_argument(
        "--target_resolution_ms",
        type=float,
    ...
    run_analysis_for_intervals(
        args.input_file,
        args.output_folder,
        args.target_resolution_ms,
        args.num_intervals,
    )
