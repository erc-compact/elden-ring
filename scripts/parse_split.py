...
def read_candidate_files(files, chunk_id, ouput_dir, pulsarx_threads=20, pepoch_override=None, start_frac=None, end_frac=None, config_file=None, verbose=False):
    ...
    df_candidates = df_candidates.astype({"snr": float, "dm": float, "period": float, "nh": int,
                                          "acc": float, "nassoc": int, "cand_id_in_file": int})
    
    if verbose:
        print(f"{len(df_candidates)} candidates read.")
        
    logging.info(f"Dumping the candidates to unfiltered_df_for_folding.csv")
    df_candidates.to_csv(f"{ouput_dir}/unfiltered_for_folding.csv", index=False, float_format='%.18f')
    
    # If a config file is provided, filter the dataframe accordingly
    if config_file:
        logging.info(f"XML file contains {len(df_candidates)} candidates before filtering.")
        ...
        logging.info(f"After filtering, {len(df_candidates)} candidates remain for folding.")
    else:
        logging.info(f"No configuration file provided, folding all {len(df_candidates)} candidates.")
    ...
    df_candidates.to_csv(f"{ouput_dir}/filtered_candidates_file.csv", index=False, float_format='%.18f')
...
def apply_folding_configuration(df: pd.DataFrame, config_file: str) -> pd.DataFrame:
    ...
    for filter_def in filters:
        ...
        conditions.append(df['period'].between(filter_def['spin_period']['min'],filter_def['spin_period']['max']))
        conditions.append(df['snr'].between(filter_def['fft_snr']['min'],filter_def['fft_snr']['max']))
        conditions.append(df['nh'].between(filter_def['nh']['min'],filter_def['nh']['max']))
...
def fold_with_pulsarx(
    ...
    nbins, binplan, subint_length, nsubband, utc_beam,
...
def main():
    ...
    parser.add_argument('-i', '--input_file', help='Name of the input xml file',
                        type=str, nargs='+')
    parser.add_argument('-ck', '--chunk_id', help='Chunk ID for the input file',
                        type=str, default='10')
    ...
    parser.add_argument('-u', '--nbins_default', help='Default number of bins',
                        type=int, default=32)
    parser.add_argument('-l', '--binplan', help='Binplan for pulsarx.',
                        type=str, default='0.005 32 0.01 64 0.1 128')
    ...
    parser.add_argument('-rfi', '--rfi_filter', help='RFI filter value',
                        type=str, default='')
    parser.add_argument('--source_name', help='Source name to be used for folding',
    ...
    parser.add_argument('-p', '--pulsarx_fold_template', help='Fold template pulsarx',
                        type=str)
    parser.add_argument('--template_dir', help = 'Template directory with meerkat and effelsberg templates', type=str)
    parser.add_argument('--telescope', help='Telescope name (e.g., meerkat, effelsberg)',
                        type=str, default='meerkat')
    ...
    parser.add_argument('--cands_per_node', type=int, default=0,
    ...
    cands, meta = read_candidate_files(args.input_file, args.chunk_id, args.output_path, args.pulsarx_threads, args.pepoch_override, args.start_frac, args.end_frac, args.config_file, args.verbose)
    ...
    try:
        if args.fold_technique == 'pulsarx':
            ...
            if args.subint_length is None:
                ...
                print(f"Subint length not provided, using default value of {subint_length} seconds")
            else:
                subint_length = args.subint_length
            generate_pulsarX_cand_file(cands, meta, args.output_path, args.beam_name, args.cands_per_node)
